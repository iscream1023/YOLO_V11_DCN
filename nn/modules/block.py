...
class C2f(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        ...
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(int((2 + n) * self.c), c2, 1)  # optional act=FReLU(c2)
...
class Bottleneck_DCN(nn.Module):
    ...
    # [핵심] 순서가 중요합니다! 
    # (c1, c2, shortcut, g, k, e) 이 순서를 지켜야 
    # C3k_DCN에서 호출할 때 shortcut(True)이 제자리를 찾아갑니다.
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        
        # k가 정수로 들어올 때와 튜플로 들어올 때를 모두 대비
        if isinstance(k, int):
            k = (k, k)
        ...
        # 두 번째 Conv (DCN 적용 여부 결정)
        # 커널 사이즈가 1보다 클 때만 DCN을 씁니다.
        if k[1] > 1:
            # DCNv2 호출 (여기서 g는 반드시 정수여야 함)
            self.cv2 = DCNv2(c_, c2, k=k[1], s=1, g=g)
        else:
            # 일반 Conv 호출 (g=g로 명시해서 확실하게 정수를 넘김)
...
class C3k_DCN(C3k):
    """
    C3k block with DCN Bottlenecks.
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        # 기존 Bottleneck 대신 Bottleneck_DCN을 사용하도록 교체
        self.m = nn.Sequential(*(Bottleneck_DCN(c_, c_, shortcut, g, k=(3, 3), e=1.0) for _ in range(n)))


class C3k2_DCN(C3k2):
    """
    Faster Implementation of CSP Bottleneck with 2 convolutions, using DCN.
    """
    # [수정 1] 인자에 attn=False 추가 (원본 C3k2와 형상 맞춤)
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, attn=False, g=1, shortcut=True):
        
        # [수정 2] super 호출 시 attn 포함하여 순서대로 전달
        super().__init__(c1, c2, n, c3k, e, attn, g, shortcut)
        
        # C3k 대신 C3k_DCN을 사용하도록 교체
        # (여기서 g는 정수 1로 정상 전달됩니다)
        self.m = nn.Sequential(*(C3k_DCN(self.c, self.c, 2, shortcut, g) for _ in range(n)))