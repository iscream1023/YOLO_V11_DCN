...
from ultralytics.nn.modules.block import eca_layer
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C3k2_DCN:
                c1, c2 = ch[f], args[0]
            # 채널 스케일링 (width_multiple 적용)
                if c2 != nc:
                    c2 = make_divisible(c2 * width, 8)
            # args 리스트 재구성: [입력채널, 출력채널, 나머지인자들...]
                args = [c1, c2, *args[1:]]
                if m in (C3k2, C3k2_DCN): # C3k2 계열은 n을 인자로 받음
                    args.insert(2, n)
                    n = 1
        ...
        elif m is eca_layer: # 사용자 정의 ECA 레이어 처리 로직 추가
            c1 = ch[f] # 이전 레이어의 출력 채널을 입력으로 받음
            c2 = c1    # ECA는 채널 수를 변경하지 않음 (Attention 방식)
            
            # YAML에서 [k_size] 형태로 인자를 넘겨줄 경우 처리
            # 예: - [-1, 1, eca_layer, [3]] -> args는 [3]임
            k_size = args[0] if args else 3 # 인자가 없으면 기본값 3 사용
            args = [c1, k_size] # 클래스 __init__(self, channel, k_size)에 맞춤
